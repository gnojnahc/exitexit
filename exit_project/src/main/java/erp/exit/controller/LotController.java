...
public class LotController {
	...
	// 자재 등록 팝업실행
	@GetMapping("/reg")
	public String registerGet(LotVO vo, Model md) {
		log.info(" lot별 검사결과 입력창 띄움 ..");
		return "/lot/LotReg";
	}
	...
	// 자재수정 코드& 검사항목 조회
		 ...
		 * // 자재 수정기능
		 * 
		 * @PostMapping("/mod") public String modifyPost(InfoItemVO vo, Model md) {
		 * log.info("infoitem 자재 수정중.."); log.info(vo);
		 * 
		 * service.modify(vo);
		 * 
		 * md.addAttribute("ServiceCheck", "success"); md.addAttribute("SeviceCheck",
		 * "fail");
		 * 
		 * return "/infoinspectitem/InfoItemMod"; }
		 */
